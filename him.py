...
def chat():
    ...
    dp = session.get("dp") or "/static/uploads/2.jpg"
    if not session.get('logged_in'):
        return redirect(url_for('login'))
...
def handle_join(data):
    ...
    if room not in chat_history:
        chat_history[room] = []
    socketio.emit("message", {"message": f"{username} has joined the room."}, room=room)
...
def handle_message(data):
    ...
    username = data.get("username")
    dp = data.get("dp")  # Profile picture URL
    room = data.get("room")
    message = data.get("message")

    if not all([username, room, message]):
        print(f"Error: Missing data - Username: {username}, Room: {room}, Message: {message}")
        return
     # Debug extracted values
    print(f"Received - Username: {username}, Room: {room}, Message: {message}, DP: {dp}")

    dp = dp or "/static//uploads/2.jpg"
    # Store message in chat history
    chat_entry = {"username": username, "dp": dp, "message": message}
    chat_history.setdefault(room, []).append(chat_entry)  # Store messages by room
